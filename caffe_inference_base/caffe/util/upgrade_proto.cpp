...
bool UpgradeNetAsNeeded(const std::string& param_file, NetParameter* param) {
  ...
  if (NetNeedsV0ToV1Upgrade(*param)) {
    ...
    // upgrade it.
	  BOOST_LOG_TRIVIAL(info) << "Attempting to upgrade input file specified using deprecated "
              << "V0LayerParameter: " << param_file;
    NetParameter original_param(*param);
    if (!UpgradeV0Net(original_param, param)) {
      success = false;
	  BOOST_LOG_TRIVIAL(error) << "Warning: had one or more problems upgrading "
          << "V0NetParameter to NetParameter (see above); continuing anyway.";
    } else {
		BOOST_LOG_TRIVIAL(info) << "Successfully upgraded file specified using deprecated "
                << "V0LayerParameter";
    }
	BOOST_LOG_TRIVIAL(warning) << "Note that future Caffe releases will not support "
  ...
  if (NetNeedsDataUpgrade(*param)) {
	...
	BOOST_LOG_TRIVIAL(warning) << "Note that future Caffe releases will only support "
  ...
  if (NetNeedsBatchNormUpgrade(*param)) {
   BOOST_LOG_TRIVIAL(info) << "Attempting to upgrade batch norm layers using deprecated "
              << "params: " << param_file;
    UpgradeNetBatchNorm(param);
   BOOST_LOG_TRIVIAL(info) << "Successfully upgraded batch norm layers using deprecated "
              << "params.";
  }
  return success;
}
  ...
  for (int i = 0; i < param.layers_size(); ++i) {
    ...
    for (int j = 0; j < layer_connection.bottom_size(); ++j) {
      ...
      if (blob_name_to_last_top_idx.find(blob_name) ==
        ...
        BOOST_LOG_TRIVIAL(fatal) << "Unknown blob input " << blob_name << " to layer " << j;
      }
      ...
      if (source_layer.layer().type() == "padding") {
		...
		{
			BOOST_LOG_TRIVIAL(error) << "Padding layer input to "
				"non-convolutional / non-pooling layer type "
				<< layer_param.type();
			return;
		}
		if (layer_connection.bottom_size() != 1)
		{
			BOOST_LOG_TRIVIAL(error) << "Conv Layer takes a single blob as input.";
			return;
		}
		if (source_layer.bottom_size() != 1)
		{
			BOOST_LOG_TRIVIAL(error) << "Padding Layer takes a single blob as input.";
			return;
		}
		if (source_layer.top_size() != 1)
		{
			BOOST_LOG_TRIVIAL(error) << "Padding Layer produces a single blob as output.";
			return;
		}
  ...
  if (v0_layer_connection.has_layer()) {
    ...
    if (v0_layer_param.has_num_output()) {
      if (type == "conv") {
        ...
        BOOST_LOG_TRIVIAL(error) << "Unknown parameter num_output for layer type " << type;
    ...
    if (v0_layer_param.has_biasterm()) {
      if (type == "conv") {
        ...
        BOOST_LOG_TRIVIAL(error) << "Unknown parameter biasterm for layer type " << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_weight_filler()) {
      if (type == "conv") {
        ...
        BOOST_LOG_TRIVIAL(error) << "Unknown parameter weight_filler for layer type " << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_bias_filler()) {
      if (type == "conv") {
        ...
        BOOST_LOG_TRIVIAL(error) << "Unknown parameter bias_filler for layer type " << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_pad()) {
      if (type == "conv") {
        ...
        BOOST_LOG_TRIVIAL(error) << "Unknown parameter pad for layer type " << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_kernelsize()) {
      if (type == "conv") {
        ...
        BOOST_LOG_TRIVIAL(error) << "Unknown parameter kernelsize for layer type " << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_group()) {
      if (type == "conv") {
        ...
        BOOST_LOG_TRIVIAL(error) << "Unknown parameter group for layer type " << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_stride()) {
      if (type == "conv") {
        ...
        BOOST_LOG_TRIVIAL(error) << "Unknown parameter stride for layer type " << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_pool()) {
      if (type == "pool") {
        ...
        case V0LayerParameter_PoolMethod_STOCHASTIC:
          ...
          break;
        default:
          BOOST_LOG_TRIVIAL(error) << "Unknown pool method " << pool;
          is_fully_compatible = false;
        }
      } else {
        BOOST_LOG_TRIVIAL(error) << "Unknown parameter pool for layer type " << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_dropout_ratio()) {
      if (type == "dropout") {
        ...
        BOOST_LOG_TRIVIAL(error) << "Unknown parameter dropout_ratio for layer type " << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_local_size()) {
      if (type == "lrn") {
        ...
        BOOST_LOG_TRIVIAL(error) << "Unknown parameter local_size for layer type " << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_alpha()) {
      if (type == "lrn") {
        ...
        BOOST_LOG_TRIVIAL(error) << "Unknown parameter alpha for layer type " << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_beta()) {
      if (type == "lrn") {
        ...
        BOOST_LOG_TRIVIAL(error) << "Unknown parameter beta for layer type " << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_k()) {
      if (type == "lrn") {
        ...
        BOOST_LOG_TRIVIAL(error) << "Unknown parameter k for layer type " << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_source()) {
      if (type == "data") {
        ...
        BOOST_LOG_TRIVIAL(error) << "Unknown parameter source for layer type " << type;
        is_fully_compatible = false;
      }
    }
    ...
    if (v0_layer_param.has_batchsize()) {
      if (type == "data") {
        ...
        BOOST_LOG_TRIVIAL(error) << "Unknown parameter batchsize for layer type " << type;
        is_fully_compatible = false;
      }
    }
    ...
    if (v0_layer_param.has_rand_skip()) {
      if (type == "data") {
        ...
        BOOST_LOG_TRIVIAL(error) << "Unknown parameter rand_skip for layer type " << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_shuffle_images()) {
      if (type == "images") {
        ...
        BOOST_LOG_TRIVIAL(error) << "Unknown parameter shuffle for layer type " << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_new_height()) {
      if (type == "images") {
        ...
        BOOST_LOG_TRIVIAL(error) << "Unknown parameter new_height for layer type " << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_new_width()) {
      if (type == "images") {
        ...
        BOOST_LOG_TRIVIAL(error) << "Unknown parameter new_width for layer type " << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_concat_dim()) {
      if (type == "concat") {
        ...
        BOOST_LOG_TRIVIAL(error) << "Unknown parameter concat_dim for layer type " << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_det_fg_threshold()) {
      if (type == "window_data") {
        ...
        BOOST_LOG_TRIVIAL(error) << "Unknown parameter det_fg_threshold for layer type "
                   << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_det_bg_threshold()) {
      if (type == "window_data") {
        ...
        BOOST_LOG_TRIVIAL(error) << "Unknown parameter det_bg_threshold for layer type "
                   << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_det_fg_fraction()) {
      if (type == "window_data") {
        ...
        BOOST_LOG_TRIVIAL(error) << "Unknown parameter det_fg_fraction for layer type "
                   << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_det_context_pad()) {
      if (type == "window_data") {
        ...
        BOOST_LOG_TRIVIAL(error) << "Unknown parameter det_context_pad for layer type "
                   << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_det_crop_mode()) {
      if (type == "window_data") {
        ...
        BOOST_LOG_TRIVIAL(error) << "Unknown parameter det_crop_mode for layer type "
                   << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_hdf5_output_param()) {
      if (type == "hdf5_output") {
        ...
        BOOST_LOG_TRIVIAL(error) << "Unknown parameter hdf5_output_param for layer type "
                   << type;
        is_fully_compatible = false;
      }
    }
...
V1LayerParameter_LayerType UpgradeV0LayerType(const std::string& type) {
  if (type == "accuracy") {
    ...
    return V1LayerParameter_LayerType_WINDOW_DATA;
	  ...
	  BOOST_LOG_TRIVIAL(fatal) << "Unknown layer name: " << type;
...
bool UpgradeV1Net(const NetParameter& v1_net_param, NetParameter* net_param) {
  if (v1_net_param.layer_size() > 0) {
	  BOOST_LOG_TRIVIAL(fatal) << "Refusing to upgrade inconsistent NetParameter input; "
  ...
  for (int i = 0; i < v1_net_param.layers_size(); ++i) {
    if (!UpgradeV1LayerParameter(v1_net_param.layers(i),
                                 net_param->add_layer())) {
		BOOST_LOG_TRIVIAL(error) << "Upgrade of input layer " << i << " failed.";
      is_fully_compatible = false;
    }
  }
  return is_fully_compatible;
...
bool UpgradeV1LayerParameter(const V1LayerParameter& v1_layer_param,
  ...
  for (int i = 0; i < v1_layer_param.blob_share_mode_size(); ++i) {
    ...
    case V1LayerParameter_DimCheckMode_PERMISSIVE:
      ...
      break;
    default:
		BOOST_LOG_TRIVIAL(fatal) << "Unknown blob_share_mode: "
  ...
  if (v1_layer_param.has_layer()) {
	  BOOST_LOG_TRIVIAL(error) << "Input NetParameter has V0 layer -- ignoring.";
    is_fully_compatible = false;
  }
  return is_fully_compatible;
}
...
const char* UpgradeV1LayerType(const V1LayerParameter_LayerType type) {
  ...
  case V1LayerParameter_LayerType_THRESHOLD:
    return "Threshold";
  default:
	  BOOST_LOG_TRIVIAL(fatal) << "Unknown V1LayerParameter layer type: " << type;
    return "";
  }
}
...
bool UpgradeSolverType(SolverParameter* solver_param) {
  ...
  if (solver_param->has_solver_type()) {
    ...
    case SolverParameter_SolverType_ADAM:
      ...
      break;
    default:
		BOOST_LOG_TRIVIAL(fatal) << "Unknown SolverParameter solver_type: " << type;
    }
    solver_param->set_type(type);
    solver_param->clear_solver_type();
  } else {
	  BOOST_LOG_TRIVIAL(error) << "Warning: solver type already up to date. ";
    return false;
  }
  return true;
}
