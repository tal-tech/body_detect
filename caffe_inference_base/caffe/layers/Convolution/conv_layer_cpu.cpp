...
namespace facethink {
  ...
  template <typename Dtype>
  void ConvLayer<Dtype>::ForwardShape() {
    this->CheckBlobs();
    this->CheckWeights();
  ...
  template <typename Dtype>
  void ConvLayer<Dtype>::ForwardComputation() {
    this->CheckWeights();
    ...
    for (int n = 0; n < num; ++n) {
      ...
      this->forward_cpu_gemm(input_data + n * input_dim,
       			     weight_data,
       			     output_data + n * output_dim);
      
      if (this->has_bias_term()) {	
	const Dtype* bias_data = this->weights_[1]->cpu_data();
        this->forward_cpu_bias(output_data + n * output_dim, bias_data);
      }
    }
  }

  template <typename Dtype>
  void ConvLayer<Dtype>::forward_cpu_gemm(const Dtype* input, const Dtype* weights,
					  Dtype* output) {
  ...
  template <typename Dtype>
  void ConvLayer<Dtype>::conv_im2col_cpu(const Dtype* data, Dtype* col_buff){      
  ...
  template <typename Dtype>
  void ConvLayer<Dtype>::forward_cpu_bias(Dtype* output, const Dtype* bias){
    caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans,
			  this->num_output_,
			  out_spatial_dim_,
			  1,
