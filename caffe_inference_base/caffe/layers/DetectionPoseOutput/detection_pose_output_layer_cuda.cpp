...
namespace facethink {
  ...
  void CUDADetectionPoseOutputLayer<Dtype>::ForwardComputation() {
    const int    loc_count  = this->inputs_[0]->count();
    const Dtype* loc_data   = this->inputs_[0]->gpu_data();
    const int    conf_count = this->inputs_[1]->count();
    const Dtype* conf_data  = this->inputs_[1]->gpu_data();
    const int    pose_count = this->inputs_[2]->count();
    const Dtype* pose_data  = this->inputs_[2]->gpu_data();
    const Dtype* prior_data = this->inputs_[3]->gpu_data();

    // Decode Pose
    Dtype* pose_permute_data = this->pose_permute_.mutable_gpu_data();
    PermuteDataGPU<Dtype>(pose_count,
			  pose_data,
    ...
    // Decode predictions.
    Dtype* bbox_data = this->bbox_preds_.mutable_gpu_data();
 
    const bool clip_bbox = true;
    DecodeBBoxesGPU<Dtype>(loc_count,
			   loc_data,
			   prior_data,
			   this->code_type_,
    ...
    if (!this->share_location_) {
      Dtype* bbox_permute_data = this->bbox_permute_.mutable_gpu_data();
      PermuteDataGPU<Dtype>(loc_count,
			    bbox_data,
			    this->num_loc_classes_,
			    this->num_priors_,
			    4,
    ...
    Dtype* conf_permute_data = this->conf_permute_.mutable_gpu_data();
    PermuteDataGPU<Dtype>(conf_count,
			  conf_data,
			  this->num_classes_,
			  this->num_priors_,
			  1,
  ...
  INSTANTIATE_CLASS(CUDADetectionPoseOutputLayer);
} // namespace facethink
