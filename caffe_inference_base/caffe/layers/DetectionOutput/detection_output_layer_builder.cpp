#include "caffe/layers/DetectionOutput/detection_output_layer_builder.hpp"

#ifndef CPU_ONLY
#include "caffe/layers/DetectionOutput/detection_output_layer_cuda.hpp"
#else
...
namespace facethink {
  ...
  DetectionOutputLayerBuilder<Dtype>::Create(const caffe::LayerParameter& layer_param) {
    std::string name;
    int num_classes;
    float nms_threshold;
    int top_k;
...
#ifndef CPU_ONLY
    layer = std::make_shared<CUDADetectionOutputLayer<Dtype> >(num_classes,
							       nms_threshold,
							       top_k,
							       confidence_threshold,
							       keep_top_k,
							       ...
							       name);
#else			
    layer = std::make_shared<DetectionOutputLayer<Dtype> >(num_classes,
							   nms_threshold,
							   top_k,
							   confidence_threshold,
							   keep_top_k,
  ...
  template <typename Dtype>
  void DetectionOutputLayerBuilder<Dtype>::ParseLayerParameters(const caffe::LayerParameter& layer_param,
								int& num_classes,
								float& nms_threshold,
								int& top_k,
								float& confidence_threshold,
    ...
    const caffe::DetectionOutputParameter& do_param = layer_param.detection_output_param();

    if (!do_param.has_num_classes()){
      BOOST_LOG_TRIVIAL(error)<< "DetectionOutLayerBuilder: must specify num_classes";
    }
	...
	BOOST_LOG_TRIVIAL(error)<< "DetectionOutLayerBuilder: unsupported code type.";
      }
    }else{
      code_type = CodeType::CORNER;
    }
    ...
    if (!nms_param.has_top_k()){
      BOOST_LOG_TRIVIAL(error)<< "DetectionOutLayerBuilder: must specify top_k";
    }
